...
- [x] **FILT-01**: Paper is eligible only if its arXiv category is in the configured topic list (`cs.AI`, `cs.CL`)
- [x] **FILT-02**: Paper is eligible only if any survey keyword matches case-insensitive in title or abstract
- [x] **FILT-03**: Survey keyword list is fixed and includes the provided phrases (e.g., "survey", "review", "state of the art", "taxonomy")
- [x] **FILT-04**: Only eligible papers are sent to the Telegram channel
- [x] **FILT-05**: Message format for eligible papers remains unchanged from current output
